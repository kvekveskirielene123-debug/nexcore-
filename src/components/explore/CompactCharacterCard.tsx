...
export function CompactCharacterCard({
  ...
  return (
    <Link
      ...
      className="group relative block rounded-[14px] overflow-hidden border bg-[#0c0520]/80"
      style={{
        aspectRatio: "4/5",
        minWidth: 140,
        maxWidth: 220,
        borderColor: hovered ? `rgba(${palette.glowRgb},0.45)` : "rgba(124,58,237,0.2)",
        boxShadow: hovered
          ? `0 20px 44px rgba(0,0,0,0.5), 0 0 28px rgba(${palette.glowRgb},0.12), 0 0 0 1px rgba(${palette.glowRgb},0.08)`
          : "none",
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(90deg, transparent, rgba(${palette.glowRgb},0.5), transparent)`,
      ...
      <button
        ...
        className="absolute top-2 right-2 z-10 w-7 h-7 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-sm hover:bg-black/70 transition-all duration-200"
        style={{
          boxShadow: favorited ? "0 0 8px rgba(0,229,255,0.4)" : "none",
        }}
      >
        <svg
      ...
      >
        ...
        {character.avatar_url ? (
          ...
          <span
            className="relative z-10 text-[56px] font-black transition-all duration-300"
            style={{
              fontFamily: "var(--font-display)",
              color: palette.fg,
              textShadow: hovered
                ? `0 0 32px ${palette.glow}, 0 0 8px ${palette.glow}`
        ...
        {/* CHAT → CTA overlay — fades in on hover */}
        <div
          className="absolute inset-0 flex items-end justify-center pb-3 pointer-events-none transition-opacity duration-250"
          style={{
        ...
        >
          <span
            className="text-[9px] tracking-[3px] font-bold px-2.5 py-1 rounded-full border transition-all duration-200"
            style={{
              fontFamily: "var(--font-mono)",
              color: palette.fg,
              borderColor: `rgba(${palette.glowRgb},0.45)`,
              background: `rgba(${palette.glowRgb},0.12)`,
              boxShadow: `0 0 10px rgba(${palette.glowRgb},0.3)`,
