...
/* ═══════════════════════════════════════════════════════════
   Heartbeat logo — pulsing heart with orbiting nodes
══════════════════════════════════════════════════════════════ */
    ...
    <div className="relative inline-flex items-center justify-center mb-5">
      <div
        ...
        style={{
          ...
          background: "radial-gradient(circle, rgba(167,139,250,0.12) 0%, rgba(0,229,255,0.05) 50%, transparent 70%)",
        }}
      />
      <svg width="88" height="84" viewBox="0 0 88 84" fill="none" aria-hidden>
        {/* Outer dashed orbit ring */}
        <ellipse cx="44" cy="42" rx="38" ry="36"
          stroke="rgba(124,58,237,0.18)" strokeWidth="0.8" strokeDasharray="3 7"
          className="settings-logo-orbit" />
        ...
        {([
          ...
          <circle key={i} cx={cx} cy={cy} r="2.2"
            fill="rgba(167,139,250,0.8)" className="settings-logo-node"
            style={{ animationDelay: d }} />
        ))}

        {/* Heart shape */}
        <path
          d="M44 62 C32 52 16 44 16 32 C16 24 22 18 30 18 C36 18 41 22 44 26 C47 22 52 18 58 18 C66 18 72 24 72 32 C72 44 56 52 44 62Z"
        ...
        {/* Inner heart pulse fill */}
        <path
          d="M44 62 C32 52 16 44 16 32 C16 24 22 18 30 18 C36 18 41 22 44 26 C47 22 52 18 58 18 C66 18 72 24 72 32 C72 44 56 52 44 62Z"
          fill="rgba(167,139,250,0.0)"
        >
          <animate attributeName="fill-opacity" values="0;0.12;0" dur="1.8s" repeatCount="indefinite" />
        </path>

        {/* ECG signal line across heart */}
        <polyline
          points="22,36 28,36 31,28 35,44 39,34 43,38 47,36 50,36 53,28 57,44 61,36 66,36"
          stroke="rgba(0,229,255,0.85)" strokeWidth="1.4"
...
function FavCard({ c, idx }: { c: FavCharacter; idx: number }) {
  return (
    <div
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.border = "1px solid rgba(167,139,250,0.45)";
        (e.currentTarget as HTMLElement).style.boxShadow = "0 0 24px rgba(167,139,250,0.12)";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.border = "1px solid rgba(124,58,237,0.18)";
    ...
    >
      ...
      <Link href={`/character/${c.id}`} className="block relative aspect-square overflow-hidden bg-[#0c0720]">
        {c.avatar_url ? (
          ...
          <img
            src={c.avatar_url} alt={c.name}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <span className="text-3xl font-black" style={{ fontFamily: "var(--font-display)", color: "rgba(167,139,250,0.6)" }}>
        ...
        <div
          ...
          style={{ height: "1.5px", background: "rgba(167,139,250,0.4)", animation: "card-scan 3s ease-in-out infinite", top: 0 }}
        />
      ...
      <div className="p-3">
        <Link href={`/character/${c.id}`}>
          <div
            className="text-[13px] font-bold tracking-[1.5px] truncate transition-colors duration-200 group-hover:text-[#a78bfa]"
            style={{ fontFamily: "var(--font-display)", color: "rgba(226,217,243,0.9)" }}
          >
            {c.name}
          </div>
          <div
            className="text-[9px] tracking-[1.5px] uppercase mt-0.5 truncate"
          ...
          {c.subtitle && (
            <p
              className="text-[11px] italic mt-1.5 line-clamp-2 leading-relaxed"
        ...
        <Link
          href={`/chat/${c.id}`}
          className="mt-3 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[9px] tracking-[2px] font-bold uppercase transition-all duration-200 hover:scale-[1.03]"
          style={{
...
export function FavoritesClient({ characters }: { characters: FavCharacter[] }) {
    ...
    <div className="min-h-screen bg-[#05020d] px-4 md:px-8 pt-8 pb-28 md:pb-8">
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <header className="text-center mb-8 settings-card-enter" style={{ animationDelay: "0s" }}>
          ...
          {/* FAVORITES letter-wave */}
          <div className="flex items-center justify-center gap-[2px] mb-2">
            {LETTERS.map((l, i) => (
              <span
                key={i}
                className="settings-letter text-[28px] md:text-[38px] font-black"
                style={{
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            >
              ...
              <span
                ...
                style={{ fontFamily: "var(--font-display)", color: "rgba(167,139,250,0.9)" }}
              >
                {characters.length}
              </span>
              <span
        ...
        {characters.length > 0 && (
          <div className="relative mb-6 settings-card-enter" style={{ animationDelay: "0.08s" }}>
            <svg
              ...
              width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(167,139,250,0.5)" strokeWidth="2"
            >
              <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
            </svg>
            <input
              type="text"
              placeholder="Search saved entities…"
              value={query}
            ...
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] transition-opacity hover:opacity-100 opacity-60"
        ...
        {characters.length === 0 ? (
          <div className="text-center py-20 settings-card-enter" style={{ animationDelay: "0.1s" }}>
            <div
              ...
              style={{ fontFamily: "var(--font-mono)", color: "rgba(167,139,250,0.3)" }}
            >
              NO ENTITIES DESIGNATED
            </div>
            <p
              className="text-[14px] italic mb-8 max-w-xs mx-auto leading-relaxed"
          ...
          <div className="text-center py-16 settings-card-enter">
